...
def get_clusters(nodes, lookup):
    ...
    def assign_cluster(node, cluster_number):
        # Skip applying 'in_cluster' to the cluster head (the current node)
        clusters[node.node.name] = cluster_number

        # Assign the 'in_cluster' attribute to descendants, skipping the root
        ...
        # Apply the 'in_cluster' attribute to the descendants of the current node
        if node.left:
...
def wrapper_simulation(config):
    ...
    for tree_index in range(1, num_trees + 1):
        ...
        # Apply .in_cluster attribute to all nodes under the initial nodes list
        # for node_name, cluster_number in clusters.items():
        #     node = lookup[node_name]
        #     node.node.add_feature("in_cluster", True)
        #     for n in node.node.traverse():
        #         n.add_feature("in_cluster", True)
        # Traverse the tree from the root and add the in_cluster attribute to relevant nodes
        # for node in random_tree.iter_descendants():
        #     if node.name in clusters:
        #         print(node.name)
        #         node.add_feature("in_cluster", True)
        ...
        # Set all branch lengths to 0
        set_branch_lengths_to_one(random_tree, n=0)

        # Set branch lengths of nodes with .in_cluster set to True to 1
        for node in random_tree.traverse():
            if hasattr(node, "in_cluster") and node.in_cluster:
                node.dist = 1
        # for node in random_tree.traverse():
        #     if hasattr(node, "in_cluster") and node.in_cluster:
        #         print(f"Node {node.name} is in a cluster.")
        #     else:
        #         print(f"Node {node.name} is NOT in a cluster.")
